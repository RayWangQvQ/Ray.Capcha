...
namespace RECAPTCHA
{
    ...
    public class ImageProcess
    {
        ...
        /// <param name="e"></param>
        public static Bitmap RemoveBgSimple(Bitmap img)
        {
            //key 颜色  value颜色对应的数量
        ...
        /// <returns></returns>
        public static List<Bitmap> CutImage(Image img, int ww, int hh, bool is4Chars = true, int charWidth = 10, int miniHeight = 10)
        {
            Bitmap bmp = null;
            List<Boundary> list = GetImgBoundaryList(img, ref bmp, is4Chars, charWidth, miniHeight);
        ...
        /// <summary>
        /// 边缘检测分割算法（确定是四字符会有精细处理）
        /// </summary>
        /// <param name="img"></param>
        /// <param name="_bmp"></param>
        /// <param name="is4Chars"></param>
        /// <returns></returns>
        public static List<Boundary> GetImgBoundaryList(Image img, ref Bitmap _bmp, bool is4Chars, int charWidth = 10, int miniHeight = 10)
        {
            ...
            try
            {
                ...
                {
                    //X轴的起点、终点
                    startX = GetStartX(img, startX);
                    if (startX < 0) break;
                    endX = GetEndX(img, startX + 1);

                    //Y轴的起点、终点
                    startY = GetStartY(img, startX, endX);
                    int _startY = startY;
                    /*
                    bool flag = false;
                    while (!flag && _startY < img.Height)
                    {
                        endY = GetEndY(img, _startY, startX, endX);
                        flag = endY - _startY >= 5;//最小高度
                        _startY++;
                    }
                    */
                    endY = GetEndY(img, _startY, startX, endX);

                    if (endY > 0 && endY - startY >= miniHeight)//最小高度
                    {
                ...
                {
                    list = LessThanFourChars(img, out _bmp, list, charWidth);
                }
                else
                {
                    _bmp = bmp;
                ...
                {
                    Boundary boundary = tempBdList[i];
                    //最小宽度比较
                    if (
                        (boundary.EndY - boundary.StartY >= charWidth)
                        && (boundary.EndX - boundary.StartX >= 5)
                        )
                    {
                        continue;
                    }
        ...
        /// <returns></returns>
        private static List<Boundary> LessThanFourChars(Image img, out Bitmap _bmp, List<Boundary> list, int charWidth)
        {
            if (list.Count == 0)
            {
                _bmp = TrimBmp(img);
        ...
        /// </summary>
        /// <param name="img"></param>
        /// <param name="searchStartX"></param>
        /// <returns></returns>
        public static int GetStartX(Image img, int searchStartX)
        {
            Bitmap bmp = new Bitmap(img);
            int startX = 0;

            for (int x = searchStartX; x < bmp.Width; x++)
            {
                for (int y = 0; y < bmp.Height; y++)
                {
                    //遍历各个像素，获得bmp位图每个像素的RGB对象
                    Color pixelColor = bmp.GetPixel(x, y);
                    if (pixelColor.Name == "ffffffff") continue;
        ...
        /// <returns></returns>
        private static int GetEndX(Image img, int start)
        {
            Bitmap bmp = new Bitmap(img);
            int endB = 0;
        ...
        /// <returns></returns>
        private static int GetStartY(Image img, int startX, int endX)
        {
            Bitmap bmp = new Bitmap(img);
            int startY = 0;
            for (int y = 0; y < img.Height; y++)
            {
                for (int x = startX; x < endX; x++)
                {
                    ...
                    if (pixelColor.Name == "ffffffff") continue;//纯白
        ...
        /// <param name="endX"></param>
        /// <returns></returns>
        private static int GetEndY(Image img, int startY, int startX, int endX)
        {
            Bitmap bmp = new Bitmap(img);
            int endY = 0;

            /*
            for (int y = startY + 1; y < bmp.Height; y++)
            {
                int cnt = 0;
                for (int x = startX; x < endX; x++)
                {
                ...
                if (cnt == endX - startX && y - startY > 4)//防止把i的点也算进去
                {
                    endY = y;
                    break;
                }
            ...
            */
            ...
            for (int y = bmp.Height - 1; y > startY; y--)
            {
                bool isFind = false;
                for (int x = startX; x < endX; x++)
                {
